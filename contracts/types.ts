...
export interface GroundFrame { source_to_world: number[][]; plane_normal_source: [number, number, number]; plane_offset_source: number; inlier_count: number; sample_count: number; inlier_indices?: number[]; coverage: number; rms_residual: number; normal_uncertainty_rad: number; axis_uncertainty_rad?: number; evidence_kind: 'target' | 'scene' | 'manual'; evidence_ids: string[]; evidence_producer: string; evidence_author?: string | null; evidence_reason?: string | null; source_revision: string }
export interface ScaleResolution { evidence_id: string; source_revision: string; kind: 'target' | 'measured' | 'manual'; measured_length: number; measured_unit: 'm' | 'cm'; metres_per_source_unit: number; producer: string; author?: string | null; reason?: string | null }
export interface Calibration extends ArtifactBase { kind: 'calibration'; scale: 'metric' | 'arbitrary'; world_unit: 'm' | 'arbitrary'; cameras: CameraCalibration[]; ground_z?: number | null; ground_status?: 'resolved' | 'unresolved'; scale_status?: 'resolved' | 'unresolved'; ground_frame?: GroundFrame | null; scale_evidence_ids?: string[]; scale_resolution?: ScaleResolution | null; source_revision?: string | null; quality: Quality }
export interface RawScore { value: number; range_min: number | null; range_max: number | null; domain?: string | null }
export interface Landmark2D { name: Landmark; xy_px: [number, number] | null; raw_score?: RawScore | null; raw_visibility?: number | null; quality: Quality }
...
export interface SubjectCandidateEvidence { index: number; bbox_xyxy_px: [number, number, number, number]; detector_score: RawScore; match_cost?: number | null }
...
export type SemanticEditOperation =
  | { kind: 'boundary'; target_id: string; interval: Interval }
  | { kind: 'keyframe_add'; keyframe: Keyframe }
  | { kind: 'keyframe_move'; target_id: string; global_seconds: number }
  | { kind: 'keyframe_remove'; target_id: string }
export interface ManualEdits extends ArtifactBase { kind: 'manual_edits'; automatic_semantics_id: string; edits?: ManualEdit[]; automatic_manifest_sha256?: string | null; automatic_parser_revision?: string | null; session_id?: string | null; revision?: number | null; base_revision?: number | null; state_revision?: number | null; parent_state_revision?: number | null; command?: 'apply' | 'undo' | 'reset' | null; operations?: SemanticEditOperation[]; source?: string | null; author?: string | null; reason?: string | null }
export interface EffectiveSemanticView { schema_version: '1.0.0'; session_id: string; revision: number; automatic_semantics_id: string; automatic_manifest_sha256: string; manual_edits_id: string | null; origin: 'automatic' | 'manual'; validation_ground_truth: false; semantics: Semantics }
